...
下面用 Redis SDS（Simple Dynamic String）展示 Algocode 如何优化高频字符串追加路径，同时保持字节级输出行为一致。

示例工作负载通过 `sdscatprintf(s, "line %d: hello world\\n", i)` 连续追加 100000 行。格式很简单，但原始实现每次都会进入 libc 的 `vsnprintf`。

### 优化前

代码：[优化前.cpp](docs/code/%E4%BC%98%E5%8C%96%E5%89%8D.cpp)

原实现即使面对简单格式，也要计算临时缓冲区大小、复制 `va_list`、调用 `vsnprintf`，并在输出可能被截断时扩容后重新格式化。

### 优化后

优化版本为受限格式增加了快速路径。它只处理字面量、`%%`、`%d` 和 `%s`，在一次遍历中完成格式校验、长度计算和结果写入；输出超过栈缓冲区时精确扩容并写入 SDS。含 flag、width、precision 或 length modifier 的复杂格式仍回退到 `vsnprintf`。

代码：[优化后.cpp](docs/code/%E4%BC%98%E5%8C%96%E5%90%8E.cpp)

| 项目 | 优化前 | 优化后 |
|---|---|---|
| 简单格式 | 始终调用 `vsnprintf` | 识别 `%d`、`%s`、`%%` 快速路径 |
| 栈内输出 | 格式化后再追加 | 一次校验、计数并写入固定缓冲区 |
| 大输出 | 反复扩容和重新格式化 | 精确计算长度，一次扩容后写入 |
| 复杂格式 | 使用 `vsnprintf` | 保持原样回退到 `vsnprintf` |
| 可观察行为 | 与 libc 格式化一致 | 字节级保持一致 |

Algocode 会在隔离 Worktree 中生成该候选，并通过 Correctness、Contract 和 Benchmark 验证：

- 参考实现与候选在固定格式、边界输入和错误路径上保持一致。
- `sdscatprintf` 的输出字节、长度和可观察 SDS 字段不变。
- Benchmark 比较真实批量追加性能，而不是依赖模型判断。
- 用户检查 Review、Diff 和证据后，再决定是否执行 `apply`。
...
- 待优化项目应是 Git 仓库，入口程序能够直接构建和运行（非必要，algocode如果检测到不是git仓库会自动git init）。
...
简单来说就是，将代码单独放在一个文件夹中，没有依赖，没有makelist，然后用任何编译器，能编译运行，有明确的输入和固定的输出（不能是随机数）就是可优化代码
